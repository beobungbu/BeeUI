#!/usr/bin/env bash
# R10.12 (#241) — installs this reproduction app's own dependencies from freshly
# packed BeeUI tarballs. BeeUI is UNPUBLISHED (see ../../llms.txt STATUS and
# docs/decisions/011-distribution-architecture.md): there is no `@beemvp/beeui-*`
# package on npm, so the app consumes real `pnpm pack` tarballs through the same
# package boundary CI's verify-web-consumer.sh uses — never a workspace:* link
# and never a hand-copied dist/ folder.
#
...
# examples/README.md + examples/web-consumer. See the report's gap list.
set -euo pipefail
...
if node -e "require.resolve('expo')" >/dev/null 2>&1; then
  echo "This reproduction app unexpectedly resolves the Expo runtime; it must stay independent of the Showcase's Expo path." >&2
  exit 1
fi

echo "==> Setup complete. Run: npm run build  (and: npm run typecheck)"
