...
// harness) against the live Showcase build served by `serve-showcase.mjs`,
// and measures real bounding boxes.
//
// It measures the dedicated Dynamic Type runtime fixture screen
// (`apps/showcase/runtime-smoke/dynamic-type-acceptance.tsx`) — the same
// surface the native Android evidence path measures
// (`scripts/runtime-smoke/android-dynamic-type.sh`), so Web and native
// evidence describe identical component instances, one tap from Showcase
// home, without any Component Gallery traversal.
//
...
//      not just `rem`-sized text, which is the distinction `docs/dynamic-type.md`
//      states accurately. Using `zoom` here is what makes this page-zoom
//      evidence, not another root-font-size run under a different name.
//
// Coverage: the two components #143 changed (`SelectTrigger`,
// `PaginationItem` — both switched `h-*` to `min-h-*`) and two representative
// fixed-height exceptions from `FIXED_HEIGHT_ALLOWLIST` (`Button`, `Input`,
// both `h-control-*`).
//
// Evidence class — stated honestly: this is real Chromium/Web layout
// evidence. It does not exercise iOS/Android native text rendering (the
// Android emulator path in `scripts/runtime-smoke/android-dynamic-type.sh`
// covers the representative native axis; see docs/dynamic-type.md's Evidence
// section) and it does not exercise every component on the audited surface —
// only the four-representative set. It also does not, by itself, distinguish
// `min-h-*` from a reverted bare `h-*` on Web (see the inline comment on the
...
const targets: Record<TargetName, { testId: string; label: string }> = {
  'select-trigger': {
    testId: 'dynamic-type-select-trigger',
...
async function openDynamicTypeFixture(page: Page) {
  await page.goto(showcaseBaseUrl, { waitUntil: 'load' });
  await page.getByTestId('showcase-open-dynamic-type').click();
  await page.getByTestId('dynamic-type-ready').waitFor({ state: 'visible' });
}
...
test.describe('Dynamic Type / font-scaling — real browser evidence (#143)', () => {
  test('root font-size scaling (OS/browser default text-size proxy): rendered height grows and content is never clipped', async ({ page }) => {
    ...
    await openDynamicTypeFixture(page);
    ...
    for (const scale of SCALE_STEPS) {
      ...
      //
      // Honest limitation, verified by hand on the original #269 evidence
      // run: this assertion alone does NOT distinguish `min-h-11`/`min-h-10`
      // from a reverted bare `h-11`/`h-10`. Every BeeUI Web sizing token
      // (`h-*` and `min-h-*` alike) and every semantic typography role
      ...
      // asserting the literal class string — and what the Android evidence
      // path proves with real OS-rendered bounds. This real-browser test
      // proves a different, complementary thing (actual scaled rendering
      // happens and is never clipped), not a substitute for those guards.
  ...
  test('CSS zoom scaling (real browser page-zoom evidence, distinct from rem/root-font-size): all four controls scale and remain unclipped', async ({ page }, testInfo) => {
    ...
    for (const scale of SCALE_STEPS) {
      ...
      for (const name of ALL_TARGETS) {
        ...
        // the docs/dynamic-type.md claim: zoom is a separate, broader
        // browser-level mechanism from rem-based typography, not something
        // rem "makes work". Height is the robust, monotonic proof of that
        // here: it grows at every step for every target. Width is measured
        // but deliberately not asserted to grow monotonically — for a
        // percentage/flex-basis child inside a viewport-width-constrained
        // row (as these fixtures are, at the 390px mobile viewport this
        // project uses), `zoom` recomputes the child's share of its
        // container in the zoomed coordinate space, which can make a
  ...
  test('SelectValue keeps its documented single-line truncation under real root font-size scaling', async ({ page }) => {
    ...
    const value = page.locator('[data-testid="dynamic-type-select-value"]');
