...
`Popover`, `DropdownMenu`, `Select`, `Tooltip`, `Dialog` and `Sheet` all use BeeUI's anchored-overlay runtime. On native, `Sheet` additionally requires its own `SheetProvider` mounted below `BeeUIProvider` — see [Expo: Sheet on native](/docs/start/expo/#sheet-on-native-optional) or [Bare React Native: Sheet on native](/docs/start/bare-react-native/#sheet-on-native-optional).
