...
description: "List and manage saved payment methods."
---
...
[Open the exact pattern in the real BeeUI Web Showcase](/showcase/?surface=pattern&id=payment-methods&embed=1). This is a Web runtime preview, not native-device evidence.

This screen exposes a single default state.

The same source is available at [`apps/showcase/patterns/dashboard-finance/screens/payment-methods-screen.tsx`](https://github.com/beobungbu/BeeUI/blob/main/apps/showcase/patterns/dashboard-finance/screens/payment-methods-screen.tsx).

## Composition

Principal public BeeUI exports used by this screen: [`Badge`](/docs/components/badge/), [`Button`](/docs/components/button/), [`Card`](/docs/components/card/), [`HStack`](/docs/components/stack/), [`Text`](/docs/components/text/), [`VStack`](/docs/components/stack/).
...
Read from the screen file and the 2 pattern-local files in its runtime import closure, direct and transitive: `dashboard-finance/screens/payment-methods-screen.tsx`, `dashboard-finance/components/screen-shell.tsx`, `dashboard-finance/components/section-header.tsx`. Every fact below is scoped to those files and to nothing else.

- **BeeUI layout primitives rendered:** `Box` (`dashboard-finance/components/screen-shell.tsx`), `HStack` (`dashboard-finance/components/section-header.tsx`, `dashboard-finance/screens/payment-methods-screen.tsx`), `VStack` (`dashboard-finance/components/section-header.tsx`, `dashboard-finance/screens/payment-methods-screen.tsx`).
- **Scroll ownership:** `ScrollView` (`dashboard-finance/components/screen-shell.tsx`).
- **Horizontal scrolling:** no `horizontal` scroll container in `dashboard-finance/screens/payment-methods-screen.tsx`, `dashboard-finance/components/screen-shell.tsx`, `dashboard-finance/components/section-header.tsx`.
- **Width constraint:** `max-w-6xl` (`dashboard-finance/components/screen-shell.tsx`).
- **Breakpoint-prefixed utility classes:** none in `dashboard-finance/screens/payment-methods-screen.tsx`, `dashboard-finance/components/screen-shell.tsx`, `dashboard-finance/components/section-header.tsx`.
- **Platform-prefixed utility classes:** none in `dashboard-finance/screens/payment-methods-screen.tsx`, `dashboard-finance/components/screen-shell.tsx`, `dashboard-finance/components/section-header.tsx`.
- **Platform branching:** no `Platform.OS` or `Platform.select` call in `dashboard-finance/screens/payment-methods-screen.tsx`, `dashboard-finance/components/screen-shell.tsx`, `dashboard-finance/components/section-header.tsx`.
- **Viewport measurement:** no `useWindowDimensions`, `Dimensions.get` or breakpoint hook in `dashboard-finance/screens/payment-methods-screen.tsx`, `dashboard-finance/components/screen-shell.tsx`, `dashboard-finance/components/section-header.tsx`.
...
- **Roles this screen sets itself:** none set in `dashboard-finance/screens/payment-methods-screen.tsx`, `dashboard-finance/components/screen-shell.tsx`, `dashboard-finance/components/section-header.tsx`.
- **Accessibility states and properties it sets itself:** none set in `dashboard-finance/screens/payment-methods-screen.tsx`, `dashboard-finance/components/screen-shell.tsx`, `dashboard-finance/components/section-header.tsx`.
...
**Intentionally excluded:** Payment-provider integration and method mutations.
...
- [Source](https://github.com/beobungbu/BeeUI/blob/main/apps/showcase/patterns/dashboard-finance/screens/payment-methods-screen.tsx)
