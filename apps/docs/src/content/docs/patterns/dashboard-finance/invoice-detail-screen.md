...
Read from the screen file and the 2 pattern-local files in its runtime import closure, direct and transitive: `invoice-detail-screen.tsx`, `screen-shell.tsx`, `section-header.tsx`. Every fact below is scoped to those files and to nothing else.

- **BeeUI layout primitives rendered:** `Box` (`screen-shell.tsx`), `HStack` (`invoice-detail-screen.tsx`, `section-header.tsx`), `VStack` (`invoice-detail-screen.tsx`, `section-header.tsx`).
- **Scroll ownership:** `ScrollView` (`screen-shell.tsx`).
- **Horizontal scrolling:** no `horizontal` scroll container in `invoice-detail-screen.tsx`, `screen-shell.tsx`, `section-header.tsx`.
- **Width constraint:** `max-w-6xl` (`screen-shell.tsx`).
- **Breakpoint-prefixed utility classes:** none in `invoice-detail-screen.tsx`, `screen-shell.tsx`, `section-header.tsx`.
- **Platform-prefixed utility classes:** none in `invoice-detail-screen.tsx`, `screen-shell.tsx`, `section-header.tsx`.
- **Platform branching:** no `Platform.OS` or `Platform.select` call in `invoice-detail-screen.tsx`, `screen-shell.tsx`, `section-header.tsx`.
- **Viewport measurement:** no `useWindowDimensions`, `Dimensions.get` or breakpoint hook in `invoice-detail-screen.tsx`, `screen-shell.tsx`, `section-header.tsx`.
...
- **Roles this screen sets itself:** none set in `invoice-detail-screen.tsx`, `screen-shell.tsx`, `section-header.tsx`.
- **Accessibility states and properties it sets itself:** none set in `invoice-detail-screen.tsx`, `screen-shell.tsx`, `section-header.tsx`.
