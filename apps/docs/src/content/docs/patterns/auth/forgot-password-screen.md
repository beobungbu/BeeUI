...
Read from the screen file and the 1 pattern-local file it imports: `forgot-password-screen.tsx`, `auth-shared.tsx`. Every fact below is scoped to those files and to nothing else.
...
- **Scroll ownership:** no scroll container is rendered in `forgot-password-screen.tsx`, `auth-shared.tsx`.
