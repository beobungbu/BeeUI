...
Read from the screen file and the 1 pattern-local file it imports: `profile-setup-screen.tsx`, `auth-shared.tsx`. Every fact below is scoped to those files and to nothing else.
...
- **Scroll ownership:** no scroll container is rendered in `profile-setup-screen.tsx`, `auth-shared.tsx`.
