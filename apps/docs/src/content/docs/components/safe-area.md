...
- **Style axes:** this family declares no variant or size prop of its own; it carries `Omit<React.ComponentProps<typeof NativeSafeAreaProvider>, 'children'>`, `React.ComponentProps<typeof NativeSafeAreaView>`, whose styling props are declared there rather than here.
