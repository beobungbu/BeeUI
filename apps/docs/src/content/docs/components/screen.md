---
title: "Screen"
description: "Base application surface with semantic background and optional spacing; owns no safe-area behavior and offers only an opt-in scroll body."
---

<!-- Generated by scripts/public-component-reference.mjs. Do not hand-edit. -->

Base application surface with semantic background and optional spacing; owns no safe-area behavior and offers only an opt-in scroll body.
...
- **Source:** [`packages/ui/src/components/screen.tsx`](https://github.com/beobungbu/BeeUI/blob/main/packages/ui/src/components/screen.tsx)
...
Stateless base application surface with a semantic background and optional spacing prop; it owns no safe-area behavior itself. `scroll` defaults to `false` (children render directly, unchanged); pass `true` for a keyboard-avoiding `ScrollView` body, or a `ScrollViewProps` object to override it (e.g. `{ keyboardShouldPersistTaps: 'always' }`) — for the fuller Android focused-field scroll-into-view contract, use `KeyboardAwareScreen` instead.
...
- **Roles this family assigns:** none set in `screen.tsx`.
- **Accessibility states and properties it sets:** none set in `screen.tsx`.
...
- [`basic`](/showcase/?surface=component&id=screen&example=basic) — smallest meaningful complete usage

The Showcase links demonstrate Web behavior; use the native-preview guide for real simulator/emulator/device paths.

## Live Web preview

<div class="beeui-component-preview" data-component="screen">
  <iframe
    src="/showcase/?surface=component&id=screen&example=basic&embed=1"
    title="Live Web preview of Screen"
    loading="lazy"
...
- **Family root / primary export:** `Screen`
...
[line 474](https://github.com/beobungbu/BeeUI/blob/main/apps/showcase/component-gallery/component-gallery.tsx#L474-L474):
...
`padding` is a four-value preset applied to both axes at once: there is no per-axis, per-edge or token-level spacing control, so anything else comes from `className`. `scroll`'s keyboard-avoiding wrapper is the same basic case `KeyboardAwareScreen` covers more fully; it does not add safe-area edges of its own.
...
- [Source](https://github.com/beobungbu/BeeUI/blob/main/packages/ui/src/components/screen.tsx)
