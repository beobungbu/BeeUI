...
- **Accessibility states it manages:** `checked`, `disabled`.
...
- **Style axes:** this family declares no variant or size prop of its own; it carries `Omit<PressableProps, 'accessibilityRole' | 'role' | 'children' | 'onPress'>`, whose styling props are declared there rather than here.
