...
- **Roles this family assigns:** `button`, `region` — set in `collapsible.tsx` by the components themselves, not by the caller.
- **Accessibility states and properties it sets:** `accessibilityLabel`, `disabled`, `expanded` — read from `collapsible.tsx`.
...
````tsx
import { Collapsible, CollapsibleContent, CollapsibleTrigger, Text } from '@beemvp/beeui-ui';
````
