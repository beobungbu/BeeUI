...
- **Roles this family assigns:** `switch` — set by the components themselves, not by the caller.
...
- **Style axes:** this family declares no variant or size prop of its own; it carries `Omit<RNSwitchProps, 'accessibilityRole' | 'role' | 'ios_backgroundColor' | 'thumbColor' | 'trackColor'>`, whose styling props are declared there rather than here.
- **Class-name surfaces:** none declared by this family; any it accepts come from `Omit<RNSwitchProps, 'accessibilityRole' | 'role' | 'ios_backgroundColor' | 'thumbColor' | 'trackColor'>`.
