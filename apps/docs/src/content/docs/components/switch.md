...
- **Roles this family assigns:** `switch` — set in `switch.tsx` by the components themselves, not by the caller.
- **Accessibility states and properties it sets:** `checked`, `disabled` — read from `switch.tsx`.
...
- **Style axes:** none of its own — its appearance comes from tokens and your own classes; it also carries `Omit<RNSwitchProps, 'accessibilityRole' | 'role' | 'ios_backgroundColor' | 'thumbColor' | 'trackColor'>`.
- **Class-name surfaces:** none declared by this family; it also carries `Omit<RNSwitchProps, 'accessibilityRole' | 'role' | 'ios_backgroundColor' | 'thumbColor' | 'trackColor'>`.
