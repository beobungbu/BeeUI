...
| `size` | `'sm' \| 'md' \| 'lg'` | `'md'` | Selects one of `progressVariants`'s preset style sets, declared in `packages/ui/src/components/progress.tsx` — the classes each value applies are there. |
