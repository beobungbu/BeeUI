...
| `padding` | `'none' \| 'sm' \| 'md' \| 'lg'` | `'md'` | Chooses this element's `padding` from `cardVariants`'s presets, declared in `packages/ui/src/components/card.tsx` — the classes each value applies are there. |
| `variant` | `'surface' \| 'outlined' \| 'raised' \| 'muted'` | `'outlined'` | Chooses this element's `variant` from `cardVariants`'s presets, declared in `packages/ui/src/components/card.tsx` — the classes each value applies are there. |
...
- **Roles this family assigns:** none set in `card.tsx`.
- **Accessibility states and properties it sets:** none set in `card.tsx`.
...
- **Style axes:** `padding` (4 values), `variant` (4 values) — the values are in the props tables above.
...
[lines 529–565](https://github.com/beobungbu/BeeUI/blob/main/apps/showcase/component-gallery/component-gallery.tsx#L529-L565):
...
[lines 1046–1082](https://github.com/beobungbu/BeeUI/blob/main/apps/showcase/component-gallery/component-gallery.tsx#L1046-L1082):
