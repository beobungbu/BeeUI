...
- **Accessibility states it manages:** `hidden`, `label`, `selected`, `sort`.
...
- **Style axes:** this family declares no variant or size prop of its own; it carries `Omit<React.ComponentProps<typeof Text>, 'children'>`, `Omit<ViewProps, 'children'>`, whose styling props are declared there rather than here.
