...
- **Roles this family assigns:** `button`, `checkbox`, `radio`, `radiogroup` — set by the components themselves, not by the caller.
- **Accessibility states and properties it sets:** `accessibilityLabel`, `checked`, `disabled`, `selected`.
