...
- **Roles this family assigns:** none set in `visually-hidden.tsx`; each element keeps the role of the primitive it renders.
- **Accessibility states and properties it sets:** none set in `visually-hidden.tsx`.
...
- **Class-name surfaces:** none declared by this family; it also carries `ViewProps`.
