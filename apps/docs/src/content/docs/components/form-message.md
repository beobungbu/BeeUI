---
title: "Form Message"
description: "Destructive form feedback (with muted HelperText variant) using polite live-region semantics."
---
...
- **Source:** [`packages/ui/src/components/form-message.tsx`](https://github.com/beobungbu/BeeUI/blob/main/packages/ui/src/components/form-message.tsx)
...
Stateless destructive form-feedback text with polite live-region semantics by default; it renders the muted `HelperText` variant when not carrying an active error.
...
- **Roles this family assigns:** none set in `form-message.tsx`; each element keeps the role of the primitive it renders.
- **Accessibility states and properties it sets:** `accessibilityLiveRegion` — read from `form-message.tsx`.
...
- **Style axes:** none of its own — its appearance comes from tokens and your own classes; it also carries `RNTextProps`.
...
- [`basic`](/showcase/?surface=component&id=form-message&example=basic) — smallest meaningful complete usage

The Showcase links demonstrate Web behavior; use the native-preview guide for real simulator/emulator/device paths.

## Live Web preview

<div class="beeui-component-preview" data-component="form-message">
  <iframe
    src="/showcase/?surface=component&id=form-message&example=basic&embed=1"
    title="Live Web preview of Form Message"
    loading="lazy"
...
- [Source](https://github.com/beobungbu/BeeUI/blob/main/packages/ui/src/components/form-message.tsx)
