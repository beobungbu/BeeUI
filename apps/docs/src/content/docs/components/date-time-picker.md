...
- **Accessibility states and properties it sets:** `accessibilityHint`, `accessibilityLabel`, `accessibilityLabelledBy`, `disabled`, `expanded` (Web), `hidden` — read from `date-time-picker.native.tsx`, `date-time-picker.web.tsx`, `date-time-picker-shared.tsx`.
