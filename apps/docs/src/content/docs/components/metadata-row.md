---
title: "Metadata Row"
description: "Read-only label/value metadata row primitive."
---
...
- **Source:** [`packages/ui/src/components/metadata-row.tsx`](https://github.com/beobungbu/BeeUI/blob/main/packages/ui/src/components/metadata-row.tsx)

## Import

```tsx
import { MetadataRow } from '@beemvp/beeui-ui';
```

There is no documented deep/private source import. For source ownership from a BeeUI checkout:

```bash
pnpm beeui add metadata-row
```
...
Stateless read-only label/value row primitive with no data-state or formatting ownership.
...
- **Roles this family assigns:** none set in `metadata-row.tsx`; each element keeps the role of the primitive it renders.
- **Accessibility states and properties it sets:** none set in `metadata-row.tsx`.
...
- [`basic`](/showcase/?surface=component&id=metadata-row&example=basic) — smallest meaningful complete usage

The Showcase links demonstrate Web behavior; use the native-preview guide for real simulator/emulator/device paths.

## Live Web preview

<div class="beeui-component-preview" data-component="metadata-row">
  <iframe
    src="/showcase/?surface=component&id=metadata-row&example=basic&embed=1"
    title="Live Web preview of Metadata Row"
    loading="lazy"
...
- **Family root / primary export:** `MetadataRow`
...
- [Source](https://github.com/beobungbu/BeeUI/blob/main/packages/ui/src/components/metadata-row.tsx)
