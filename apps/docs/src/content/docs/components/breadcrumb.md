---
title: "Breadcrumb"
description: "Router-neutral breadcrumb trail with decorative separators hidden from accessibility."
---
...
- **Source:** [`packages/ui/src/components/breadcrumb.tsx`](https://github.com/beobungbu/BeeUI/blob/main/packages/ui/src/components/breadcrumb.tsx)

## Import

```tsx
import { Breadcrumb, BreadcrumbItem } from '@beemvp/beeui-ui';
```

There is no documented deep/private source import. For source ownership from a BeeUI checkout:

```bash
pnpm beeui add breadcrumb
```
...
Stateless router-neutral trail; separators between `BreadcrumbItem`s are decorative and hidden from accessibility, `BreadcrumbItem` renders link semantics for navigable ancestors and non-interactive current-location semantics for the active item, and supplied child keys are preserved when separators are composed in.
...
- **Roles this family assigns:** `link`, `text` — set in `breadcrumb.tsx` by the components themselves, not by the caller.
- **Accessibility states and properties it sets:** `accessibilityElementsHidden`, `accessibilityLabel`, `accessible`, `disabled`, `selected` — read from `breadcrumb.tsx`.
...
- **Primary executable fixture:** [`apps/showcase/__tests__/logical-direction.test.tsx`](https://github.com/beobungbu/BeeUI/blob/main/apps/showcase/__tests__/logical-direction.test.tsx)
...
- [`basic`](/showcase/?surface=component&id=breadcrumb&example=basic) — smallest meaningful complete usage
- [`states`](/showcase/?surface=component&id=breadcrumb&example=states) — the states example

The Showcase links demonstrate Web behavior; use the native-preview guide for real simulator/emulator/device paths.

## Live Web preview

<div class="beeui-component-preview" data-component="breadcrumb">
  <iframe
    src="/showcase/?surface=component&id=breadcrumb&example=basic&embed=1"
    title="Live Web preview of Breadcrumb"
    loading="lazy"
...
- **Family root / primary export:** `Breadcrumb`
...
These are the parts of the typechecked **runtime Showcase fixture behind this live preview** — [`apps/showcase/component-gallery/component-gallery.tsx`](https://github.com/beobungbu/BeeUI/blob/main/apps/showcase/component-gallery/component-gallery.tsx), 1074 lines — where **Breadcrumb** is actually used: 4 lines in 1 place. Each block is copied verbatim from the line range named above it, so it is the same executable source, not a retelling of it. The rest of that file exercises other families and is not reproduced here.

[lines 956–959](https://github.com/beobungbu/BeeUI/blob/main/apps/showcase/component-gallery/component-gallery.tsx#L956-L959):
...
- [Source](https://github.com/beobungbu/BeeUI/blob/main/packages/ui/src/components/breadcrumb.tsx)
