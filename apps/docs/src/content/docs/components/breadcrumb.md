...
- **Style axes:** this family declares no variant or size prop of its own; it carries `Omit<PressableProps, 'accessibilityRole' | 'children' | 'role'>`, `Omit<ViewProps, 'children'>`, whose styling props are declared there rather than here.
