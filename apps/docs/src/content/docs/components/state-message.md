...
- **Roles this family assigns:** none set in `state-message.tsx`.
- **Accessibility states and properties it sets:** `accessible` — read from `state-message.tsx`.
...
````tsx
import { Button, EmptyState, ErrorState, Separator } from '@beemvp/beeui-ui';
````
