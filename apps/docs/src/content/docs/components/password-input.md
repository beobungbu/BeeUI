...
- **Roles this family assigns:** none set in `password-input.tsx`; each element keeps the role of the primitive it renders.
...
- **Style axes:** `size` (3 values, inherited from `InputProps`) — the values are in the props tables above.
