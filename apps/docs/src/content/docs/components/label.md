---
title: "Label"
description: "Semantic form/control label with accessible required-state wording and optional nativeID linkage."
---
...
```tsx
import { Label } from '@beemvp/beeui-ui';
```
...
- Primary export: `Label`
  - Also routed here, outside the Registry family:
    - `label`
  - Package export subpath: `@beemvp/beeui-ui/label`

**Exported types:** `LabelProps`
...
Stateless form/control label; `required` renders accessible required-state wording, and `nativeID` links it to a control only when explicitly supplied.
...
- **Roles this family assigns:** none set in `label.tsx`; each element keeps the role of the primitive it renders.
- **Accessibility states and properties it sets:** `accessibilityElementsHidden`, `accessibilityLabel`, `hidden` — read from `label.tsx`.
...
- **Style axes:** `tone` (8 values, inherited from `TextProps`) — the values are in the props tables above.
...
- [`basic`](/showcase/?surface=component&id=label&example=basic) — smallest meaningful complete usage

The Showcase links demonstrate Web behavior; use the native-preview guide for real simulator/emulator/device paths.

## Live Web preview

<div class="beeui-component-preview" data-component="label">
  <iframe
    src="/showcase/?surface=component&id=label&example=basic&embed=1"
    title="Live Web preview of Label"
    loading="lazy"
...
- **Family root / primary export:** `Label`
...
- [Source](https://github.com/beobungbu/BeeUI/blob/main/packages/ui/src/components/label.tsx)
