...
- **Accessibility states and properties it sets:** `accessibilityHint`, `accessibilityLabel`, `accessibilityLabelledBy`, `disabled`, `expanded` (Web), `hidden` — read from `date-picker.native.tsx`, `date-picker.web.tsx`, `date-picker-shared.tsx`.
