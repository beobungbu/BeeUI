...
| `align` | `DatePickerAlign` | `'start'` | Web-only: `Popover` content alignment relative to the trigger. Ignored on native. |
| `className` | `string` | — | Extra utility classes, merged after the component's own via `cn(...)`, so they win on conflict. An escape hatch for source-owned and application work, not a cross-engine portability guarantee. |
| `clearAccessibilityLabel` | `string` | `'Clear date'` | Accessible label for the clear button shown when `clearable` and a value is selected. Defaults to `'Clear date'`. |
...
| `collisionPadding` | `DatePickerCollisionPadding` | `8` | Web-only: `Popover` collision padding. Ignored on native. |
...
- **Roles this family assigns:** `button` (iOS and Android) — set in `date-picker.native.tsx` by the components themselves, not by the caller.
- **Accessibility states and properties it sets:** `accessibilityHint`, `accessibilityLabel`, `accessibilityLabelledBy`, `disabled`, `expanded` (Web), `hidden`, `required` (Web) — read from `date-picker.native.tsx`, `date-picker.web.tsx`, `date-picker-shared.tsx`.
