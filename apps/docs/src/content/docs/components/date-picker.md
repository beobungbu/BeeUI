...
- **Roles this family assigns:** `button` — set in `date-picker.d.ts`, `date-picker.native.tsx`, `date-picker.web.tsx`, `date-picker-shared.tsx`, `date-picker-locale.ts` by the components themselves, not by the caller.
- **Accessibility states and properties it sets:** `accessibilityHint`, `accessibilityLabel`, `accessibilityLabelledBy`, `disabled`, `expanded`, `hidden` — read from `date-picker.d.ts`, `date-picker.native.tsx`, `date-picker.web.tsx`, `date-picker-shared.tsx`, `date-picker-locale.ts`.
