...
- **Roles this family assigns:** none set in `icon-button.tsx`; each element keeps the role of the primitive it renders.
...
- **Style axes:** `variant` (5 values, inherited from `ButtonProps`) — the values are in the props tables above.
