...
| `size` | `'sm' \| 'md' \| 'lg' \| 'xl'` | `'md'` | Selects one of `avatarVariants`'s preset style sets, declared in `packages/ui/src/components/avatar.tsx` — the classes each value applies are there. |
