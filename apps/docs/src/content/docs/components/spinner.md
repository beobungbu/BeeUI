...
- **Roles this family assigns:** none set in `spinner.tsx`.
- **Accessibility states and properties it sets:** `accessibilityLabel`, `accessibilityLabelledBy` — read from `spinner.tsx`.
...
````tsx
import { Box, Spinner } from '@beemvp/beeui-ui';
````
