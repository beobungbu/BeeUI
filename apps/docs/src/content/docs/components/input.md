...
| `size` | `'sm' \| 'md' \| 'lg'` | `'md'` | Selects one of `inputVariants`'s preset style sets, declared in `packages/ui/src/components/input.tsx` — the classes each value applies are there. |
