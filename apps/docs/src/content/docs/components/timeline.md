...
The terminal-connector flag comes from a context the timeline places around each of its own direct children, so a `TimelineItem` nested inside a wrapper element, or rendered outside a `Timeline`, is treated as not-last and draws a connector below itself.
