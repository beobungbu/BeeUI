...
- **Roles this family assigns:** `tab`, `tablist`, `tabpanel` — set by the components themselves, not by the caller.
...
- **Style axes:** this family declares no variant or size prop of its own; it carries `Omit<PressableProps, 'accessibilityRole' | 'role' | 'children' | 'onPress'>`, `Omit<ViewProps, 'accessibilityRole' | 'role'>`, `Omit<ViewProps, 'children' | 'role'>`, `Omit<ViewProps, 'children'>`, whose styling props are declared there rather than here.
