...
- **Roles this family assigns:** `link` — set in `link.tsx` by the components themselves, not by the caller.
- **Accessibility states and properties it sets:** `accessibilityLabel`, `disabled` — read from `link.tsx`.
...
- **Style axes:** none of its own — its appearance comes from tokens and your own classes; it also carries `Omit<PressableProps, 'accessibilityRole' | 'children' | 'role'>`.
...
````tsx
import { Link, Text, VStack } from '@beemvp/beeui-ui';
````
