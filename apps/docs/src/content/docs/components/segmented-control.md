...
description: "Compact mutually-exclusive selection surface with radiogroup semantics."
---
...
- **Style axes:** none of its own — its appearance comes from tokens and your own classes; it also carries `Omit<PressableProps, 'accessibilityRole' | 'children' | 'onPress' | 'role'>`, `Omit<ViewProps, 'children' | 'role'>`.
...
- **Additional fixture:** [`apps/showcase/__tests__/segmented-control-name-and-label-wrap.test.tsx`](https://github.com/beobungbu/BeeUI/blob/main/apps/showcase/__tests__/segmented-control-name-and-label-wrap.test.tsx)
...
- [`basic`](/showcase/?surface=component&id=segmented-control&example=basic) — smallest meaningful complete usage

The Showcase links demonstrate Web behavior; use the native-preview guide for real simulator/emulator/device paths.

## Live Web preview

<div class="beeui-component-preview" data-component="segmented-control">
  <iframe
    src="/showcase/?surface=component&id=segmented-control&example=basic&embed=1"
    title="Live Web preview of Segmented Control"
    loading="lazy"
...
- **Family root / primary export:** `SegmentedControl`
...
A press on the already-selected item changes nothing: `onValueChange` fires only for an item that is not selected. An item's own `onPress` runs first but cannot cancel the selection that follows it.
...
- [Source](https://github.com/beobungbu/BeeUI/blob/main/packages/ui/src/components/segmented-control.tsx)
