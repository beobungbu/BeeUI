...
export function RuntimeAcceptance({ onBack }: { onBack: () => void }) {
  ...
  const lowerInputRef = React.useRef<React.ComponentRef<typeof Input>>(null);
  ...
  return (
    <Screen testID="runtime-smoke">
      ...
      <KeyboardAwareScreen
        ...
        testID="runtime-keyboard-screen"
      >
        <Box className="w-full gap-6 px-5 py-8">
          ...
          <Card className="gap-4" variant="raised">
            <Section
              description="Public KeyboardAwareScreen dynamic focus path under reduced usable height."
              title="Focus and scrolling"
            >
              <VStack gap="sm">
                <Button
                  onPress={() => lowerInputRef.current?.focus()}
                  testID="runtime-focus-second"
                  variant="outline"
                >
                  Focus lower input
                </Button>
                ...
                <Box className="h-24" />
                <Field label="Lower runtime input">
                  <Input
                    autoCapitalize="none"
                    placeholder="lower runtime input"
                    ref={lowerInputRef}
                    testID="runtime-input-second"
                  />
                </Field>
                <Text testID="runtime-keyboard-usable">
                  Both fields must remain reachable while the keyboard reduces usable height.
                </Text>
              </VStack>
            </Section>
          </Card>
