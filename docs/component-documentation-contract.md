...
documentation every public `@beeui/ui` component must carry, and how that coverage
...
> STATUS: BeeUI is pre-1.0 and UNPUBLISHED. No `@beeui/*` package or CLI is on npm.
...
[`@beeui/ui`](../packages/ui/src/index.ts) barrel exports and
...
| **Import** | derived | The exact `import { … } from '@beeui/ui'` line with the real exported symbols. |
...
| **Executable examples** | derived | Links to typechecked `@beeui/showcase` fixtures that import the component's symbols. |
...
`@beeui/showcase` app, a renamed export, a new component, or a removed example
...
- [Executable examples check](../scripts/check-doc-examples.mjs) — validates that
  code examples across the docs reference only real `@beeui/ui` exports (#222).
