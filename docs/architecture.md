...
Overlay content resolves against the **nearest overlay scope** — one coherent unit with three aligned responsibilities (`DialogContent`, via React Native `Modal` in its own native window, provisions one; the mechanism is generic, so future Select/Tooltip inherit it):

- **host** — where content is portaled (the modal window, not behind it);
- **geometry** — each scope measures its own host origin, so overlays inside a `pageSheet`/`formSheet` position relative to the sheet, not the root window (no hardcoded presentation offsets);
- **dismissal** — each scope has its own dismiss stack, so "topmost" is scoped: a root overlay behind a modal never steals dismissal from a modal-local child, whatever order they opened in. Outside press, accessibility escape, web Escape, and hardware back all respect the scope; global events (web Escape, Android root back) dispatch to the topmost active scope.

Platform request-close is routed deliberately: **Android** hardware back arrives via `Modal.onRequestClose` (the root back handler is suppressed) and is intercepted child-first; **iOS/other** request-close can be a native sheet-swipe dismissal, so it is **not** intercepted (that would desync React state from the gone native modal) and applies the close policy directly. `onRequestClose` fires exactly once per native request on all platforms and is never double-called with the backdrop/accessibility path.
