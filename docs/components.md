...
| `SafeArea` | layout | Explicit `react-native-safe-area-context` surface with caller-owned edge selection; BeeUI never silently adds system insets to generic screen/chrome components. A `className` padding utility on the same edge as a system inset composes with it (the inset plus the caller's own inner padding), rather than one replacing the other. |
| `Screen` | layout | Base application surface with semantic background and optional spacing; owns no safe-area behavior of its own. `scroll` (`boolean` or a `ScrollViewProps` override, default `false`) opts into a keyboard-avoiding `ScrollView` body for a long form/list with no hand-rolled wrapper; omitting it renders `children` directly, unchanged. Use `KeyboardAwareScreen` instead for the fuller Android focused-field scroll-into-view contract. |
...
| `IconButton` | action | Icon-only action; accessible label is required. `size` (`'sm'` | `'md'` | `'lg'` | `'icon'`, default `'icon'`) reuses `Button`'s own control-height tokens as both height and width for a genuinely square control per size — `'sm'`'s 36px visual box still keeps the accepted `>=44dp` tappable region on native. `count` renders a small overlay badge (a number/string is appended to the accessible label so it is announced; any other node must carry its own accessible text) anchored to the top-end corner; omitted (the default) renders no badge. |
...
| `SearchInput` | form | Search keyboard/submit semantics layered on `Input`; clearing a previously non-empty query emits one `onSearch('')` reset signal. `trailing` renders extra content (e.g. a filter button) after the input in its own row; omitted (the default) renders the input with no extra wrapper, unchanged. |
...
| `Chip` | selection | Standalone toggle or value-scoped group item with button/radio/checkbox semantics; grouped items without a value fail safe as disabled and warn in development. A standalone `Chip` with `interactive={false}` renders as a static, read-only tag — no button/radio/checkbox role and no press handling. |
| `ChipGroup` | selection | Controlled/uncontrolled single or multiple selection coordination. `allowDeselect` lets pressing the already-selected `Chip` again clear the `'single'`-mode selection instead of always leaving exactly one selected; it has no effect in `'multiple'` mode, which can already reach zero selections by unchecking every `Chip`. |
...
| `Toolbar` | navigation | Single-row action strip that measures its own width and every `ToolbarItem` child's natural width, collapsing whichever items do not fit (lowest `priority` first) into a trailing overflow `DropdownMenu`. See [Toolbar contract](#toolbar-contract). |
| `ToolbarItem` | navigation | Declares one toolbar action's `label`/`icon`/`disabled`/`onPress`/`priority`/collapse metadata; a rendering primitive only inside `Toolbar` — see [Toolbar contract](#toolbar-contract). |
...
| `TabsList` | navigation | Tablist semantic container. `scrollable` lets the strip overflow horizontally instead of every trigger stretching to fill the row (`flex-1`, the pre-`scrollable` default); on Web the newly-selected trigger also scrolls into view and participates in arrow-key roving-tabindex navigation. Defaults to false. |
| `TabsTrigger` | navigation | Accessible tab selection trigger. `closable` renders an accessible close control as a sibling of the trigger's own pressable (never nested inside it, avoiding a `Pressable`-in-`Pressable` composition) and requires `closeAccessibilityLabel`; pressing it calls `onClose` and, if this tab is currently selected, also calls `Tabs`'s own `onValueChange`. Defaults to false. |
...
| `AlertDialogContent` | modal overlay | Alert-dialog surface that never closes from backdrop presses, nor from a physical `Escape` keypress on Web (`dismissOnEscape={false}`, not caller-configurable). Native request-close paths — Android hardware back and accessibility escape — do dismiss it unless `cancelOnRequestClose={false}`. Android Modal hardware back dismisses a nested anchored child first, then follows the alert cancellation policy; iOS/other native request-close is not child-intercepted. `cancelOnRequestClose={false}` makes the no-child native request notification-only. Accessibility escape reaches the same `cancelOnRequestClose` gate as hardware back, not a path outside it. |
...
| `ListItem` | application pattern | Optional press behavior; interactive rows synthesize deterministic accessible names from primitive title/description/trailing content, while non-interactive rows group primitive content without hiding complex descendants. `active` marks this row as the current/selected item (e.g. the active sidebar entry) — a caller-owned boolean reflected as a visual/accessibility state, independent of whether the row is also interactive. |
...
`Toolbar` measures its own container width and every `ToolbarItem` child's natural width (a hidden, never-collapsed measurement pass), then moves whichever items do not fit into a trailing overflow `DropdownMenu` — lowest `priority` first, ties breaking trailing-to-leading. An item that omits `priority` never collapses, even if that means the row itself overflows. `overflowAccessibilityLabel` is required once anything actually collapses (a dev warning fires if it is missing).

`ToolbarItem` metadata is authoritative in both the row and the overflow menu: `disabled` is cloned onto the visible child (overriding whatever `disabled` the child element itself declares) and passed to the overflow menu row when collapsed; `onPress` is cloned onto the visible child only when the child does not already declare its own `onPress` — an `onPress` the child sets itself always wins there instead (a dev warning fires once if both are set to different functions), while the overflow menu row always uses the item's own `onPress`. `className` applies to the item's own wrapper in the row, not to the child element or the overflow menu row.

The overflow-fit budget accounts for the row's own gap between every rendered child (visible items and, once anything collapses, the trailing overflow trigger) and the trigger's own measured width — not a fixed 44px estimate: `Toolbar` reads the real trigger's rendered width once it has mounted at least once, falling back to the `size="icon"` control token until then. The row gap is algorithm-owned and cannot be overridden through root `style.gap`, keeping rendered spacing and collapse math identical.

On Web, one item is Tab-reachable at a time (WAI-ARIA Toolbar Pattern roving-tabindex): ArrowLeft/ArrowRight move the roving "current" slot with wrap-around (RTL-aware via `useDirection()`, ADR-004), Home/End jump to the first/last enabled slot, and the overflow trigger is always the sequence's last stop once anything has collapsed — a collapsed item itself is only reachable by opening that menu, never directly.
...
`sheet.native.tsx` replaces the skeleton's rendering on iOS/Android behind the identical public contract, wrapping the optional `@gorhom/bottom-sheet` adapter (`BottomSheetModal`) instead of RN's `<Modal>`. **Required app-root wiring** is BeeUI's public `SheetProvider`, mounted below `BeeUIProvider` (see `apps/showcase/App.tsx`). On native, `SheetProvider` owns the required `GestureHandlerRootView` + `BottomSheetModalProvider`, keeping gorhom's portal host below BeeUI's runtime contexts by construction. It deliberately does not reuse an already-present outer gorhom provider; that boundary is reported as a dev-time misconfiguration while BeeUI mounts its own inner provider. Web/fallback `SheetProvider` is a pass-through, so no native engine is loaded there.
...
- **Portal/context topology**: `SheetProvider` fixes provider ownership at the root; native gorhom content is still store-reparented, so Sheet captures BeeUI's Sheet/safe-area/overlay/toast/theme state before presentation and re-provides it inside the modal portal. `bridgeContexts` explicitly preserves consumer-owned React contexts BeeUI cannot enumerate (React Query, i18n, navigation, app stores). Toasts reuse one provider-local store and render through exactly one active/topmost Sheet-local viewport; nested anchored overlays target `ModalOverlayHost` inside the Sheet.
...
**`density`/`align`/`onPress`**: `Table`'s `density` prop (`TableDensity = 'compact' | 'comfortable' | 'spacious' | 'dense48'`, `layout="scroll"` only) overrides the row height for that one `Table`, reusing the same `--spacing-density-row-height` values (44px/56px/64px) the global application-density axis already defines, plus a fourth, table-specific 48px `'dense48'` step; omitting it leaves rows following the ambient global density exactly as before this prop existed. `TableHead`/`TableCell` accept `align` (`TableAlign = 'start' | 'center' | 'end'`) for numeric/action columns instead of a hand-written `className="items-end text-end"` combination. `TableRow`'s optional `onPress` makes the whole row a real keyboard-reachable pressable (native: `accessibilityRole="button"`; Web: the row stays a real `<tr>`, gaining `tabIndex={0}` plus `Enter`/`Space` keydown activation alongside the pointer `onClick`). On Web, activating an embedded interactive descendant (a cell `Button`, `Checkbox`, `Radio`, `SelectTrigger`, link, or any other native interactive element/ARIA widget role) never also fires the row's own `onPress` — native has no equivalent guard, so a row with `onPress` should not also embed its own interactive controls there (a screen reader exposes the whole row as one button regardless of what its cells contain); see `apps/docs/src/content/docs/components/table.md`'s own `onPress` note for the full platform-parity caveat.
