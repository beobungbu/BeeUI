...
- **Portal/context topology**: native gorhom content is store-reparented, so Sheet captures BeeUI's Sheet/safe-area/overlay/toast/theme state before presentation and re-provides it inside the modal portal. `bridgeContexts` explicitly preserves consumer-owned React contexts BeeUI cannot enumerate (React Query, i18n, navigation, app stores). Toasts reuse the provider-local store but render through a Sheet-local viewport above the modal; nested anchored overlays continue to target `ModalOverlayHost` inside the Sheet.
