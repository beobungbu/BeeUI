...
> **Candidate SHA:** `5cb061f60df312e04036c1f6108ef0f099307bd9` (`main`).
> **Candidate commit:** `ci: fail visual-web-report when any shard fails (close the masking that hid the CASE C red)`, 2026-09-02.
> **Snapshot:** 2026-09-02. Generated with Node 24.13.1, pnpm 10.15.0.

## Why this candidate (supersedes the un-landed `#401` draft)

An earlier draft freeze ([#401], never merged) tentatively bound `1.0.0-rc-ready.1` to the
prior head `a58abe71a179a395f7ace35c1e73adf8515737bc`. That surface was **red**: the
deterministic Web visual gate (`visual-web`) failed on the "Web Escape CASE C" overlay
interaction, and the failure had been partly masked by a `visual-web-report` shard that did not
fail when a shard failed. Both were fixed after that draft:

- **[#402]** (`028b82f`, `fix(overlay): route web Escape to topmost modal scope, not
  last-registered overlay (CASE C)`) corrects the web Escape routing so a shallow overlay can no
  longer steal Escape from a deeper modal scope; and
- **`5cb061f`** (this candidate's commit) makes `visual-web-report` fail when any shard fails,
  closing the masking that hid the CASE C red.

Because #402 changed packed source (`packages/ui/src/components/overlay-runtime.tsx`), the draft
candidate was invalid and is **not** used. Since the prior draft never landed on `main` (the
in-tree audit still recorded "candidate not yet frozen"), the `.1` label was never actually
consumed; this document is the **first landed freeze**, bound to the current green post-fix SHA,
and correctly carries `1.0.0-rc-ready.1`. `visual-web` is now **green on this exact candidate
SHA** ([docs/rc-ci-matrix.md](rc-ci-matrix.md), #247).
...
| Candidate SHA | `5cb061f60df312e04036c1f6108ef0f099307bd9` |
| Lockstep package version | `0.1.0` (unbumped; `1.0.0` is owner-gated at #254) |
| Machine-stamped artifact version | `0.1.0-rc-ready.5cb061f60df3` |
...
- **`0.1.0-rc-ready.5cb061f60df3`** — the version `scripts/pack-artifacts.mjs` deterministically
...
| `@beemvp/beeui-ui` | `0.1.0` | `beemvp-beeui-ui-0.1.0.tgz` | `e87eebc9b4db55efd0def4abcd8c7fd824c4912ee1be13d29d2206f78fd2dc4c` | 525103 |
| `@beemvp/beeui-cli` | `0.1.0` | `beemvp-beeui-cli-0.1.0.tgz` | `f38f064288528cb55af93ea20622da0632c41ce63f5bfe6d006295b00da7e4a3` | 189248 |

These checksums are **fresh for this candidate**: `@beemvp/beeui-ui` changed versus the
un-landed draft because of the #402 overlay-Escape fix (`e87eebc9…`, was `887ec148…`), and
`@beemvp/beeui-cli` changed because it embeds the registry source snapshot of `ui`
(`f38f0642…`, was `79b743bb…`). `@beemvp/beeui-core` (`b509450e…`) and `@beemvp/beeui-tokens`
(`b0dbbed9…`) are byte-identical to the draft — neither was touched by #402 — which is itself a
useful integrity signal.
...
```bash
git checkout 5cb061f60df312e04036c1f6108ef0f099307bd9
...
assert m['commit'] == '5cb061f60df312e04036c1f6108ef0f099307bd9'
assert m['candidateVersion'] == '0.1.0-rc-ready.5cb061f60df3'
...
(including all `@beemvp/beeui-ui` granular subpaths) from inside the tarball, installs the set
...
[#401]: https://github.com/beobungbu/BeeUI/pull/401
