...
The accepted contracts are retained across transports: non-modal positioning, shared geometry/collision handling, safe-area and keyboard policy, deterministic topmost dismissal, nested overlay behavior, accessibility semantics, and no silent conversion to a full-screen React Native `Modal`. `DialogContent` provisions a modal-local host so anchored overlays inside a dialog render in the modal window (generic — future `Select`/`Tooltip` inherit it). It also keeps a modal-local **dismissal** scope: because Android `Modal` suppresses the root back handler, hardware back reaches BeeUI only through `Modal.onRequestClose`, which dismisses the modal's topmost anchored child first (reason `back`) and only closes the Dialog once none remains — a root overlay behind the Dialog never consumes the Dialog's back event.

Regression evidence:

- jest transport contract suite: native teleport preserves context; legacy fallback drops it; capability selection; open/unmount lifecycle; Dialog → Popover; Dialog → DropdownMenu (opens, preserves context, selects, closes, dialog stays open, child-first dismissal); modal request-close child-first on Android hardware back (Dialog → DropdownMenu / Popover back #1 closes the child and keeps the Dialog, back #2 closes the Dialog; a root Popover behind the Dialog is never consumed; AlertDialog child-first + `cancelOnRequestClose`; nested Dialog scope isolation); legacy insertion-order preserved on independent portal updates; legacy host-lifecycle cleanup (mount/unmount/remount, no leaked destinations).
...
- Device runtime evidence (iOS + Android): context preserved at the root host and inside a `Dialog` modal-local host for both Popover and DropdownMenu, with correct anchored placement, menu selection, close-on-select, dialog staying visible, outside-press child-first, and Android **hardware-back** child-first (back dismisses the nested overlay first, then the Dialog).
