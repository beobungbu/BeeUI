...
| 5 | Expo SDK | `~57.0.0` (`apps/showcase/package.json`, `apps/visual-regression/package.json`) | `57.0.15` (lockfile) | Bundle/compile (Metro/Expo export, Android APK, iOS Simulator xcodebuild) + native runtime smoke | `57.0.15` → TESTED; the rest of declared `~57.0.0` → CANDIDATE until separately exercised | #130/#131 (exercised together with the RN version each proves) |
