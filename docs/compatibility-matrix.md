...
`scripts/check-compatibility-matrix.mjs` (run via `pnpm compat:check`, part of local
`pnpm typecheck` and CI's `contracts` verification lane) parses the exact pinned/tested
