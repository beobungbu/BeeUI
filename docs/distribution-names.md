...
| --- | --- | --- | --- |
| `@beeui/core` | library (D1) | 404 | **Available** |
| `@beeui/tokens` | library (D1) | 404 | **Available** |
| `@beeui/ui` | library (D1) | 404 | **Available** |
| `@beeui/cli` | CLI (recommended) | 404 | **Available** |
...
The whole `@beeui` scope is free: all three library names and `@beeui/cli` resolve 404, so
reserving the scope reserves the coherent set in one action.
...
`@beeui/core`, `@beeui/tokens`, `@beeui/ui` are **locked** as the public 1.0 library names.

- They match what the repo already ships: `packages/{core,tokens,ui}/package.json` are each
  `"name": "@beeui/*"`, `"license": "MIT"`, and the bare/Web consumer verify scripts already
...
- They are non-confusable: the scope prefix disambiguates them from the unrelated
  ...
  the `beeui` tombstone. No scoped `@beeui/*` name collides.
- ADR-011 D1 promotes `@beeui/tokens` from `private: true` to public alongside the other
  two (the Web theme import `@import '@beeui/tokens/theme.css'` and the #355 fix both need
  it resolvable by name). Removing `"private": true` and finalizing publishable metadata is
  **#199**, not this preflight.

No library name needs to change, so the `@beeui/*` **import examples throughout the docs
are already final** and are out of scope for the rename alignment below.

## CLI name — recommendation

**Recommendation: publish the CLI as the scoped package `@beeui/cli`, invoked as
`npx @beeui/cli add <component>`, exposing a `beeui` binary (`"bin": { "beeui": … }`).**
...
### `@beeui/cli` vs `create-beeui`

| Dimension | `@beeui/cli` → `npx @beeui/cli add` | `create-beeui` → `npm create beeui` |
| --- | --- | --- |
...
| Namespace coherence | Lives inside the reserved `@beeui` scope with the libraries; one scope reservation covers everything | Sits in the unscoped global namespace, disconnected from `@beeui/*` |
| Binary name | Can still be `beeui`, so end users type `beeui add button` once installed / `npx @beeui/cli add button` ad hoc | Binary convention is tied to the `create` invocation |

`create-beeui` is available and could be added **later** as a thin optional scaffolder
(`npm create beeui` → new starter app) without conflicting with `@beeui/cli`; the two are
complementary, not either/or. But the primary add-component CLI should be `@beeui/cli`.

### Binary name stays `beeui`

The published package name and the binary name are independent. `@beeui/cli` should declare
...
dropped), even though the install/package name is `@beeui/cli`. This preserves the
...
under `@beeui/*` already gives the primary protection. `bee-ui` and the `beeui` tombstone
...
**CLI invocation** wording changes; `@beeui/*` library import examples do not. Files that
...
- Root `package.json` — the `"beeui": "node ./scripts/beeui.mjs"` workspace script (repo-local
  shim; a future `packages/cli` with `"name": "@beeui/cli"` + `"bin": { "beeui": … }` is the
...
1. **Create/own the npm `@beeui` organization (scope).** Owner-only. Reserving the scope
   secures `@beeui/core`, `@beeui/tokens`, `@beeui/ui`, and `@beeui/cli` together. Publishing
   scoped packages publicly requires `--access public` (or org default access = public).
2. **Reserve the CLI package `@beeui/cli`** (and, if desired, defensively publish/hold
   `create-beeui`, `beui`, `beeui-cli`). Owner-only. Do **not** attempt to claim unscoped
   `beeui` — it is a tombstone.
3. **Grant publish permissions / team membership** on the `@beeui` org to the release
...
| --- | --- | --- | --- |
| Core library | `@beeui/core` | `import … from '@beeui/core'` | Locked, available |
| Tokens library | `@beeui/tokens` | `import … from '@beeui/tokens'` | Locked, available |
| UI library | `@beeui/ui` | `npm i @beeui/ui` | Locked, available |
| CLI | `@beeui/cli` | `npx @beeui/cli add <component>` (binary `beeui`) | **Recommended**, available |
