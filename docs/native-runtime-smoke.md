...
The script creates a temporary modern iPhone Pro simulator, boots it, performs clean Expo Prebuild, installs pods, compiles the Debug Simulator app, installs it, starts Metro, runs the common flow, the isolated #126 runtime-stress flow, and the iOS sheet flow, records sheet video/screenshots, and deletes the temporary simulator on exit.
...
The default CI device is a Pixel 7 AVD using API 36 Google APIs x86_64. `reactivecircus/android-emulator-runner` owns AVD creation/boot, adb transport, and teardown; `android.sh` consumes the already-booted emulator, performs clean Expo Prebuild, builds/installs the Debug APK, starts Metro, uses `adb reverse` for Metro, runs the common and #126 stress flows, then performs the Android-specific Back matrix.
...
| input/keyboard | focus + actual text injection; iOS uses explicit UI dismissal instead of Maestro `hideKeyboard` |
...
| I8 #126 runtime stress | root Select; outside dismiss → independent movement → reopen; modal-local Select; child Popover across real keyboard show/hide |
...
| Movement after dismissal | independent real scroll reaches labelled sentinel and lower target, then returns to trigger |
