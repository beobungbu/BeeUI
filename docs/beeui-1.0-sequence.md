...
**Current state (2026-09-02): all agent-doable BeeUI 1.0 work is COMPLETE. S0–S8 are done, the STOP at "BeeUI is 1.0-ready, not released" is reached, and the immutable RC candidate `1.0.0-rc-ready.1` (#246) is stamped at SHA `5cb061f` with a green exact-candidate CI matrix (#247). Exactly 15 issues remain open — every one owner-gated, deferred to physical devices, or the program tracker. Repository is PUBLIC (owner action 2026-08-30); branch/tag/release rulesets are live (#196).** The open set: **#114** (tracker, open by design); **#62 + #349** (documented non-blocking iOS native-simulator edge cases); **#126 + #248 + #249** (real physical-device native runtime + RC VoiceOver/TalkBack — owner-deferred to devices); **#198 + #205 + #207** (reserve the `@beemvp` npm org + trusted-publishing/provenance — owner account actions); **#254 + #255** (owner-authorized publish + verify-published); **#215** (package-manager mutation policy — owner decision); **#229** (optional MCP server — owner decision); **#234 + #253** (real-world external consumer + RC soak — owner). By release: R0–R6 complete; **R7 done** except owner-gated #198/#205/#207 (distribution ADR-011, package metadata/output shape, export maps, packed inventory, prerelease-equivalent artifacts, clean consumers, dist-tag policy, consumer-compat report all landed); **R8 done** except owner-decision #215 — the CLI is extracted, hardened and given dependency diagnostics + project/platform detection + `init` policy (#209–#214/#216), registry closure #217 landed (distribution debt [#355](https://github.com/beobungbu/BeeUI/issues/355) resolved by declaring `@beemvp/beeui-tokens` a consumer dependency on each affected registry item), and #218 (packed CLI clean-consumer E2E) + #219 (diff/update) both landed; **R9 done** except owner-decision #229 (docs site, per-component/executable-example/pattern docs, Web Showcase, native preview, the `llms.txt` family, agent cookbook + fresh-agent regression suite #220–#228); **R10 done** except owner #234 — consumer starters #230–#233, demo architecture #236, demo shell #258, demo screens #259–#263, integration epic #237, demo acceptance #238–#242 and the fresh-agent reference app #235 are all accepted; **R11 agent-work done** — API freeze #243, token freeze #244, semver audit #245, rollback runbook #256, RC candidate #246, CI matrix #247, RC Web-a11y #250, security audit #251 and changelog/migration #252 all landed, leaving only the owner/real-device gates #248/#249/#253/#254/#255. A late freeze catch — a real overlay-Escape routing bug (CASE C) that had been red-but-masked — was fixed (#402) and the `visual-web-report` required-check masking closed, so `main`'s visual matrix is now genuinely green.
