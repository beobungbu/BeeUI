...
- **Platform**: must hold identically for React Native (iOS/Android, New Architecture) and React Native Web; the two `measureInWindow` call sites are platform-agnostic source, so the contract cannot fork by `Platform.OS` inside `@beeui/ui` beyond documenting expected-frequency differences.
...
- **Source ownership / dependency**: no new npm dependency. The mechanism must live in `@beeui/ui`'s existing runtime layer (`overlay-runtime.tsx`), not in `@beeui/core`'s pure geometry kernel, which by contract (`docs/anchored-overlays.md` Layer 1) has no React/React Native/DOM/timer dependency.
