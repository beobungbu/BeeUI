...
- **Dependency/package/registry impact**: no new runtime dependency; one new small internal helper within `@beeui/ui` (e.g. co-located with `overlay-runtime.tsx`), not added to the package's public export surface. The resolver stays internal by default; it is not exported as a new public utility unless a concrete future consumer independently justifies doing so, in that consumer's own scoped decision. No required root wrapping, non-breaking.
...
4. The resolver itself stays internal to `@beeui/ui` by default. It is not added to the package's public export surface as part of this ADR; exporting it as a public utility would require its own independent justification in a future scoped decision, not an assumption baked in here.
