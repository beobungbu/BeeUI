...
- **Anchored-overlay geometry** (`@beemvp/beeui-core`) already has a real, platform-neutral direction contract. `packages/core/src/utils/anchored-overlay.ts:3` defines `AnchoredOverlayDirection = 'ltr' | 'rtl'`; `alignedCoordinate()` (`packages/core/src/utils/anchored-overlay.ts:140-158`) flips `'start'`/`'end'` alignment for top/bottom placements when `direction === 'rtl'` (`packages/core/src/utils/anchored-overlay.ts:177`); `resolveAnchoredOverlayPosition()` defaults to `direction: 'ltr'` when the caller omits it (`packages/core/src/utils/anchored-overlay.ts:244`). `docs/architecture.md:138` documents this as "logical alignment with deterministic flip/shift... and RTL rules." `@beemvp/beeui-core` has no `react`/`react-native` dependency (`packages/core/package.json`: only `clsx` and `tailwind-merge`), so it cannot and does not read any platform direction signal — direction is always an explicit input to this layer.
...
- **Reuse, don't duplicate, existing authorities**: `@beemvp/beeui-core`'s `AnchoredOverlayDirection` type and `resolveAnchoredOverlayPosition()`'s direction-aware math are already accepted and must not be reimplemented; theme/scope precedent (`BeeThemeScope`) sets the pattern to mirror, not compete with.
- **Package boundary**: `@beemvp/beeui-core` has zero `react`/`react-native` dependency (`packages/core/package.json`) and "must not import Expo or application code" (`docs/architecture.md:43`). Any platform read (`I18nManager`, `document`) must live in `@beemvp/beeui-ui` or above, never in `@beemvp/beeui-core`.
...
- **Non-goal**: this ADR does not design Table or Calendar/date-control APIs, and does not mandate that any future component expose a public `direction` override prop. It defines the internal resolver/precedence contract those future components' own implementations may call when they need JS-level direction resolution; whether a given future component also needs its own explicit public override prop is a decision for that component's own ADR (e.g. #164/#171), not this one.
...
- **Risks/tradeoffs**: directly violates the #139 hard invariant — it is a second global state engine solely for RTL, duplicating the ambient authority (`I18nManager`/DOM `dir`) instead of reading it. Requires every root to be wrapped, conflicts with `@beemvp/beeui-core`'s no-React-dependency boundary if any part leaks downward, and duplicates the "own no context, own no store" precedent `BeeThemeScope` already established for theme.
...
- **Design summary**: One pure function/hook in `@beemvp/beeui-ui` (not `@beemvp/beeui-core`, which cannot read platform APIs) resolves direction as `explicit ?? ambient ?? 'ltr'`, where the ambient read is `I18nManager.isRTL` on native and `document.documentElement.dir === 'rtl'` on Web. `Popover`, `DropdownMenu`, and `Select` call this one resolver instead of duplicating the ternary. Any future anchored-overlay-geometry component (e.g. Table, Calendar) that needs JS-level direction resolution would call the same internal resolver rather than reimplementing the read — but whether such a component also exposes its own public `direction` override prop is that component's own ADR decision, not mandated by this one. Component-level logical start/end (icon slots, alignment, margin/padding) is defined relative to the same resolved value. Subtree-level visual mirroring reuses React Native's existing Yoga `direction` style on `View` (`'ltr' | 'rtl' | 'inherit'`) — a platform mechanism BeeUI does not need to reimplement. No context, no store, no observer.
- **Benefits**: satisfies the hard invariant exactly; collapses three duplicated read-sites into one; keeps `@beemvp/beeui-core` platform-pure; keeps the existing explicit `direction` prop (already an accepted, tested-by-construction seam) as the override mechanism; mirrors the `BeeThemeScope` precedent (stateless resolver forwarding to an existing authority) instead of inventing a new pattern.
...
- **Dependency/package/registry impact**: no new runtime dependency; one new small internal helper within `@beemvp/beeui-ui` (e.g. co-located with `overlay-runtime.tsx`), not added to the package's public export surface. The resolver stays internal by default; it is not exported as a new public utility unless a concrete future consumer independently justifies doing so, in that consumer's own scoped decision. No required root wrapping, non-breaking.
- **Accessibility/RTL/large-text/reduced-motion impact**: makes the existing (currently-untested) flip/shift/align RTL math in `@beemvp/beeui-core` reachable and testable through one mechanically consistent seam, unblocking #141/#142.
...
BeeUI 1.0 adopts **Option B**: one stateless, platform-appropriate direction resolver, reused everywhere JS-level direction resolution is needed, with the existing per-component `direction` prop as the sole override seam. No new context, store, or subscription is introduced anywhere in the direction path.

### Scope boundary: resolver visibility and future component public APIs

This ADR's canonical rule, and the full extent of what it decides for components that do not exist yet:

1. BeeUI has one direction resolver/precedence contract, used wherever JS-level direction resolution is needed — today and in the future. Components do not each invent their own read of `I18nManager`/DOM `dir`.
2. The existing explicit `direction` prop overrides on `Popover`, `DropdownMenu`, and `Select` remain supported unchanged; this ADR does not touch their public API.
3. Future component ADRs (e.g. #164, #171, and any others introducing Table, Calendar, or other anchored-geometry components) decide for themselves whether their own public API needs an explicit `direction` override prop. This ADR does **not** mandate a public `direction` prop on any future Table/Calendar/date component — that is out of scope here and left to those components' own ADRs.
4. The resolver itself stays internal to `@beemvp/beeui-ui` by default. It is not added to the package's public export surface as part of this ADR; exporting it as a public utility would require its own independent justification in a future scoped decision, not an assumption baked in here.

### Direction source precedence (highest to lowest)

| Precedence | Source | Scope | Notes |
|---|---|---|---|
| 1 | Explicit per-component `direction` prop, where a component exposes one (already exists on `Popover`/`DropdownMenu`/`Select`; whether any future `Table`/`Calendar`/anchored-geometry component exposes its own is decided by that component's own ADR, not mandated here) | Single component instance | Opt-in override; always wins when present. This is the nested-scope override mechanism for anchored-overlay geometry (see below). |
...
- **Write**: BeeUI never calls `I18nManager.allowRTL()`/`forceRTL()` and never sets `document.documentElement.dir`, anywhere in `@beemvp/beeui-core` or `@beemvp/beeui-ui`. Choosing and applying the application's ambient RTL/LTR mode remains entirely the host application's responsibility — the exact same boundary `BeeThemeScope` draws for `Uniwind.setTheme()`.
...
- **Overlay geometry (`@beemvp/beeui-core`)**: unchanged, already-accepted contract. `AnchoredOverlayAlign`'s `'start'`/`'end'` values are direction-relative for `top`/`bottom` placements via `alignedCoordinate()`'s `reverseStartEnd` flip (`anchored-overlay.ts:140-158,177`). `AnchoredOverlayPlacement`'s `'left'`/`'right'` values remain physical, not logical — placement side is not flipped by `direction`, only alignment along the anchor's cross axis is. This ADR ratifies that existing, already-tested-by-construction split as final: physical placement, logical alignment. Reversing it would need new evidence, not this ADR.
...
2. **Geometry-level scoping**: the explicit `direction` prop already accepted by `Popover`/`DropdownMenu`/`Select` — and, for any future anchored-geometry component whose own ADR decides to expose one — overrides the ambient platform read for that component instance's JS geometry math.
...
- The existing explicit `direction` prop on `Popover`/`DropdownMenu`/`Select` remains the primary test seam today: RTL rendering/behavior tests pass `direction="rtl"` directly and never need to mutate global `I18nManager`/DOM state. Any future anchored-geometry component that exposes its own explicit override prop (per that component's own ADR) would gain the same test seam for free; this ADR does not require it to expose one.
- `resolveAnchoredOverlayPosition()` (`@beemvp/beeui-core`) is already a pure function taking `direction` as a parameter — no new seam needed; #141/#142 close the current test gap (no `*.test.ts*` file references `RTL`/`isRTL`/`direction` today) by adding deterministic unit tests pinning the flip/shift/align behavior `docs/architecture.md:138` already claims.
...
- **Future Table and Calendar/date controls**, if and when they need JS-level direction resolution, must consume the shared internal resolver and the logical start/end mapping rule from this ADR rather than inventing their own read of `I18nManager`/DOM `dir`. This ADR does not mandate that they expose a public `direction` override prop; whether they need one is a decision for their own component ADR (e.g. #164/#171), per the scope boundary above.
