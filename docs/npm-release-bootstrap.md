...
1. Prepare an owner-approved prerelease version `0.86.2-rc.N`: set `currentVersion` in the `json dist-tag-policy` block of `docs/dist-tag-policy.md` — the pin the release checks read — then bring root, core, tokens, ui and cli to it and merge that exact candidate to `main`. The workflow's `expected_version` default and its prerelease guard are checked against that same pin by `pnpm release-control-plane:check`.
