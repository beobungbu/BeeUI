...
// `docs/dist-tag-policy.md`'s machine-readable block is the one place the docs site's own
// publication-truth check (`scripts/check-public-doc-truth.mjs`) reads "is BeeUI on npm right
// now" from — the site pages (`start/index.md`, `release-security.md`) already read it the same
// way. The llms.txt family previously hardcoded "UNPUBLISHED" unconditionally, so once the RC
// actually published the agent-facing surfaces kept telling agents `npm install` "resolves to
// nothing today" while the human docs said the opposite (#543, #574 rows A001-A013). Falling back
// to the unpublished shape when the policy file is missing/malformed matches
// `extractPublicationPolicy`'s own fallback, so an unreadable policy fails closed (never claims a
// publication that cannot be verified).
const DEFAULT_POLICY = { published: false, currentVersion: undefined, prereleaseDistTag: 'next' };
...
export function buildModel({ registry, barrelSource, packages, policy = DEFAULT_POLICY }) {
...
const HEADER_NOTE =
  ...
  'packages/*/package.json, and docs/dist-tag-policy.md. Do not edit by hand; run `pnpm llms:generate`.';
...
## Packages
- ${packageLine(packages.core, policy)} ([packages/core/package.json](packages/core/package.json))
- ${packageLine(packages.tokens, policy)} ([packages/tokens/package.json](packages/tokens/package.json))
- ${packageLine(packages.ui, policy)} ([packages/ui/package.json](packages/ui/package.json))
...
- [docs/ai-agent-cookbook.md](docs/ai-agent-cookbook.md): AI-agent development contract + task-oriented prompt cookbook (how to consume/build/contribute; current distribution-status rules; ownership boundaries).
...
${packagesHeading}
- ${packageLine(packages.core, policy)}
- ${packageLine(packages.tokens, policy)}
- ${packageLine(packages.ui, policy)}

${packagesNote}

## Consumption models
${centralizedModel}
${sourceOwnershipModel}
...
## Runtime theme switching (app-owned system/light/dark)
Brand and density live in tokens (\`@import\` the theme, ADR-001), but the **app-level** switch between light, dark and system at runtime is owned by the application and driven through Uniwind — not a BeeUI component. The application owns exactly one preference state (BeeUI/Uniwind does not store or persist it) with three values: \`type ThemePreference = 'system' | 'light' | 'dark'\`, passed to \`Uniwind.setTheme(preference)\`. \`setTheme('light')\`/\`setTheme('dark')\` pin the app to that runtime theme regardless of what the OS/browser reports afterward; \`setTheme('system')\` is meant to release that pin and resume following the platform/browser color scheme. \`useUniwind()\` reads the current \`{ theme }\` (and \`hasAdaptiveThemes\`) so the app re-renders. The valid runtime-theme names are exported from \`@beemvp/beeui-tokens\` as \`beeRuntimeThemeNames\` (\`light\`, \`dark\`, \`violet-light\`, \`violet-dark\`).

**Known gap, verified against \`@beemvp/beeui-*@0.86.2-rc.1\` with Uniwind \`1.10.1\`:** after an explicit \`setTheme('light')\`/\`setTheme('dark')\`, a later \`setTheme('system')\` type-checks and does not throw, but the app can stay pinned to the last explicit theme — it does not reliably resume following the OS/browser scheme. Treat this as an open behavior gap in the pinned Uniwind version, not a documented contract, and resolve "system" yourself: read \`Appearance\`/\`useColorScheme\` (native) or \`window.matchMedia('(prefers-color-scheme: dark)')\` (Web) to resolve \`'system'\` to \`'light'\`/\`'dark'\` before calling \`Uniwind.setTheme\`, and re-run that resolution whenever the OS/browser scheme changes while the preference is \`'system'\`. Full pattern with a working \`useResolvedSystemScheme\`/\`applyThemePreference\` implementation: [apps/docs/src/content/docs/theming/index.md](apps/docs/src/content/docs/theming/index.md).

This three-state preference is a single small piece of app state deciding *which* value BeeUI's existing theme runtime uses — not a second theme authority, and a different concern from \`BeeThemeScope\`, which themes one subtree independently of the app-wide preference (a public \`@beemvp/beeui-ui\` export). See [docs/theming.md](docs/theming.md) and the cookbook's Recipe F in [docs/ai-agent-cookbook.md](docs/ai-agent-cookbook.md).
...
- Never nest one pressable inside another. A \`*Trigger\` (DropdownMenuTrigger, PopoverTrigger, TooltipTrigger, SheetTrigger, DialogTrigger, AlertDialogTrigger) is itself a full Button-shaped pressable — put icon/label children directly inside it, do not wrap an IconButton/Avatar in one. The same rule applies wherever a family exposes a pressable slot: \`ListItem\`'s \`trailing\` (an IconButton there is still a second nested pressable), \`SettingsItem\` composed with \`AlertDialogTrigger\`, and any other pressable-inside-pressable composition. On Web this renders one interactive element inside another, which React flags as invalid DOM nesting; use \`variant="ghost"\` on the outer pressable instead of nesting a second one.
...
- Every Props table for a component lives at \`/docs/components/<name>/\` on the docs site (e.g. \`/docs/components/list-item/\`) — this file lists exported symbols and source paths, not prop names/types/defaults; do not guess a prop's shape from the source path alone when the rendered page already documents it.
...
- \`DatePicker\`/\`DateTimePicker\` are platform-split, not native-only: \`date-picker.native.tsx\`/\`date-time-picker.native.tsx\` render the OS system picker (iOS/Android); \`date-picker.web.tsx\`/\`date-time-picker.web.tsx\` render \`Calendar\` in a \`Popover\` on Web, and work there today (ADR-008). \`Calendar\` itself is the cross-platform date-grid primitive (native + Web) when a picker's popover chrome is not wanted.
...
function loadModelFromRepo() {
  return buildModel({
    ...
    policy: extractPublicationPolicy(ROOT_DIR),
  });
}
