...
export function readPublicGuideData(rootDir = ROOT_DIR) {
  ...
  // `readPublicationState` is the single derivation path (scripts/public-site-contract-lib.mjs):
  // it already rejects legacy policy fields and derives currentVersion/prereleaseVersionPattern,
  // plus `published`/observedDistTags/state, all derived from docs/registry-observation.json
  // instead of the raw fenced-JSON block ever carrying them.
  const distribution = readPublicationState(rootDir);
