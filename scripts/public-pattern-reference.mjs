...
export function renderPublicPatternPage(pattern, rootDir = ROOT_DIR) {
  ...
  // pattern 1): once published, an `npx @beemvp/beeui-cli` invocation with no dist-tag/version
  // pin resolves to the stable `latest` channel, which does not carry a prerelease — pin it to
  // the same opt-in prerelease tag so the command is actually runnable, not just present.
  const cliInvocation = policy.published
