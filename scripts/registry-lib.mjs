...
export const REGISTRY_PATH = path.join(REPO_ROOT, 'registry', 'registry.json');
...
export async function loadRegistry({ repoRoot = REPO_ROOT, registryPath = path.join(repoRoot, 'registry', 'registry.json') } = {}) {
  let raw;
  try {
...
export async function buildAddPlan({ projectRoot, registry, config, requestedItems, overwrite = false }) {
  ...
  for (const item of items) {
    for (const file of [...item.files].sort((a, b) => configuredTarget(config, a.target).localeCompare(configuredTarget(config, b.target)))) {
      ...
      const source = resolveInside(REPO_ROOT, file.source, `source for '${item.name}'`);
      await assertNoSymlinkPath(REPO_ROOT, source, `source for '${item.name}'`);
