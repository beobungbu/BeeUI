...
const { described, distinct, sharedAcrossProps, total } = collectPropDescriptionCoverage(buildPublicComponentManifest());
...
console.log(
  `prop descriptions: ${described}/${total} (${percent}%), floor ${PROP_DESCRIPTION_FLOOR}\n` +
    `distinct descriptions: ${distinct} (floor ${PROP_DISTINCT_DESCRIPTION_FLOOR})\n` +
    `shared across different props: ${sharedAcrossProps} — reuse for the same prop name is ` +
    'correct; one sentence covering two different props is not',
);
