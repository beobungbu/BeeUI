...
function distributionStatusNote(rootDir) {
  ...
  return (
    ...
    `Stable \`latest\` currently resolves to the same RC too — npm's automatic first-publish ` +
    `default, not a deliberate promotion; it moves to a real stable version at the first stable ` +
