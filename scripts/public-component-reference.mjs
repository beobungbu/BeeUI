...
export const PROP_DESCRIPTION_FLOOR = 675;
...
export const PROP_DISTINCT_DESCRIPTION_FLOOR = 335;
...
function distributionStatusNote(rootDir) {
  ...
  return (
    ...
    `Stable \`latest\` currently resolves to the same RC too. The bootstrap publish used \`--tag next\`; ` +
    `the mechanism that also produced \`latest\` has not been established, so this is recorded as ` +
    `observed registry state rather than an npm rule. It moves to a real stable version at the first stable ` +
...
export function renderPublicComponentPage(component, rootDir = ROOT_DIR) {
  ...
  const provider = component.name === 'sheet'
