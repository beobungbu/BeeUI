...
function pressableTriggerCallout(entry) {
  ...
  return (
    '\n\n**This is the pressable itself.** It accepts every `Button` prop (`variant`, `size`, ' +
    '`loading`, `onPress`, …) — put icon/label children directly inside it. Do not nest another ' +
    'pressable (`Button`, `IconButton`, `Avatar` wrapped for press) inside a `*Trigger`: on Web ' +
    'that renders `<button>` inside `<button>`, which React flags as invalid DOM nesting. For an ' +
    'icon-only or avatar trigger, set `variant="ghost"` (and `size`/`className` as needed) on the ' +
    'trigger directly instead of wrapping a second pressable in it.'
  );
}
