...
test('Foundation manifest exposes the target IA and environment-derived origin deterministically', () => {
  ...
  assert.equal(buildDocsFoundationManifest(ROOT_DIR, { environment: 'staging' }).canonicalOrigin, 'https://beeui-stg.beemvp.com');
  assert.equal(buildDocsFoundationManifest(ROOT_DIR, { environment: 'production' }).canonicalOrigin, 'https://beeui.beemvp.com');

  const routes = new Set(first.currentDocsRoutes.map((entry) => entry.route));
  for (const route of ['/docs/', '/docs/start/', '/docs/learn/', '/docs/components/', '/docs/patterns/', '/docs/guides/', '/docs/reference/']) {
    assert.ok(routes.has(route), `missing ${route}`);
  }
});

test('release state is derived from canonical policy and exposes the public RC safely', () => {
  const state = buildReleaseState(ROOT_DIR);
  assert.equal(state.published, true);
  assert.equal(state.status, 'prerelease');
  assert.equal(state.channel, 'next');
  ...
  assert.equal(state.publicInstallCommandsAvailable, true);
  assert.equal(state.installCta, 'prerelease');
...
test('page metadata uses caller-supplied environment origin and fails closed outside production', () => {
  ...
  const production = buildPageMetadata({
    title: 'Start', description: 'Start with BeeUI.', pathname: '/docs/start/', imagePath: '/og/start.png',
    origin: productionSite.origin, environment: 'production',
  });
  ...
  const failClosed = buildPageMetadata({
    title: 'Staging', description: 'Staging metadata.', pathname: '/docs/start/', origin: stagingSite.origin, environment: 'staging',
  });
...
test('Showcase href builder is deterministic and rejects missing target identity', () => {
  assert.equal(
    buildShowcaseHref({ surface: 'component', id: 'select', ownerId: 'select', example: 'controlled', state: 'open', theme: 'dark', density: 'compact' }),
...
test('a dotted filename maps to the route Astro actually serves', () => {
  assert.equal(contentPathToRoute('apps/docs/src/content/docs/compatibility/current.generated.md'), '/docs/compatibility/currentgenerated/');
  assert.equal(contentPathToRoute('apps/docs/src/content/docs/guides/current-release.md'), '/docs/guides/current-release/');
});
...
const rule = (fromPrefix, toPrefix, aliasOf) => ({
  fromPrefix, toPrefix, status: 308, preserveSuffix: true, preserveQuery: true, ...(aliasOf ? { aliasOf } : {}),
});
...
test('several sources may converge on one real destination', () => {
  assert.deepEqual(
    collectRedirectViolations([rule('/components/', '/docs/components/'), rule('/docs/components/reference/', '/docs/components/')], { knownRoutes: KNOWN }),
    [],
  );
});
...
test('an alias naming something that is not a redirect source is rejected', () => {
  const violations = collectRedirectViolations([rule('/x/', '/docs/start/', '/docs/never-a-source/')], { knownRoutes: KNOWN });
...
test('a redirect source that shadows a more specific source is rejected', () => {
  const violations = collectRedirectViolations([rule('/docs/', '/docs/start/'), rule('/docs/getting-started/', '/docs/start/')]);
...
test('a redirect source that collides with a canonical route is rejected', () => {
  const violations = collectRedirectViolations([rule('/showcase/', '/docs/start/')], { routePrefixes: ['/', '/docs/', '/showcase/'] });
