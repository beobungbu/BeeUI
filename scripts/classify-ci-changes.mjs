...
const PACKAGE_DOC_RE = new RegExp(
  `^packages/${PACKAGE_NAMES}/(?:README\\.md|CHANGELOG\\.md|LICENSE(?:\\.md)?|docs/)`,
);

// BeeUI currently ships no native implementation. Source files with these
// extensions are package/runtime inputs only unless they match an official
// React Native Codegen spec naming convention. New native package surfaces
...
// Root package.json is intentionally absent. In this pnpm workspace, dependency
// graph changes are represented by pnpm-lock.yaml (or a package manifest).
// This lets docs/script-only root package.json edits avoid native compilers
// while frozen-lockfile install still fails closed on inconsistent dependency edits.
...
function isPackageDocumentationPath(file) {
  return PACKAGE_DOC_RE.test(file);
}

function isPackageNativeImplementationPath(file) {
  if (!isPackagePath(file) || isPackageDocumentationPath(file)) return false;
  ...
  // cpp/, react-native.config.*, codegen metadata, etc.).
  return true;
}

export function isPackageBoundarySensitivePath(value) {
  const file = normalizePath(value);
  if (!file || isPackageDocumentationPath(file)) return false;
  if (PACKAGE_BOUNDARY_EXACT_PATHS.has(file)) return true;
  if (!isPackagePath(file)) return false;
  if (PACKAGE_MANIFEST_RE.test(file) || PACKAGE_TSCONFIG_RE.test(file) || PACKAGE_SRC_RE.test(file)) {
    return true;
  }
  // Unknown non-document package surfaces fail closed.
  return true;
}
...
export function isShowcaseNativeSensitivePath(value) {
  ...
  // Unknown repository inputs fail closed. This keeps native safety for new
  // tooling/config surfaces without penalizing explicitly known docs/runtime paths.
  return true;
}
...
function writeGithubOutput(nativeResult, boundaryResult, bareResult, showcaseResult) {
  ...
  appendFileSync(process.env.GITHUB_OUTPUT, `package-boundary=${boundaryResult.packageBoundary ? 'true' : 'false'}\n`);
  appendFileSync(process.env.GITHUB_OUTPUT, `package-boundary-reason=${boundaryResult.reason.replace(/[\r\n]/g, ' ')}\n`);
  appendFileSync(process.env.GITHUB_OUTPUT, `bare-native=${bareResult.bareNative ? 'true' : 'false'}\n`);
  appendFileSync(process.env.GITHUB_OUTPUT, `bare-native-reason=${bareResult.reason.replace(/[\r\n]/g, ' ')}\n`);
  appendFileSync(process.env.GITHUB_OUTPUT, `showcase-native=${showcaseResult.showcaseNative ? 'true' : 'false'}\n`);
  appendFileSync(process.env.GITHUB_OUTPUT, `showcase-native-reason=${showcaseResult.reason.replace(/[\r\n]/g, ' ')}\n`);
  appendFileSync(process.env.GITHUB_OUTPUT, `bare-consumer=${boundaryResult.packageBoundary ? 'true' : 'false'}\n`);
}
