...
export const SOCIAL_CARD = {
  ...
  //   logo box    19.4% ink               floor 12%    (the "B" glyph and the rounded corners
  ...
  //
  // This is a "the artwork is still there" guard, not a pixel-diff. The headline and tagline
  // floors sit far below their measurements and no plausible redesign approaches them. The logo
  // box is the exception and is calibrated rather than generous: a box that keeps its square and
  // loses the glyph measures 7.1%, so a floor under that cannot tell a wordmark from a blank
  // plate. At 12% it can, but single-change redesigns land between 12.2% and 36% — a thinner or
  // smaller glyph, or dropping the plate, comes within a third of a point. The failure is loud
  // and the fix is to re-measure the floor with the new artwork; the failure this prevents is
  // silent.
