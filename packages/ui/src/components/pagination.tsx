...
export const Pagination = React.forwardRef<React.ComponentRef<typeof View>, PaginationProps>(
  ({ accessibilityLabel = 'Pagination', children, className, disabled = false, onPageChange, page, pageCount, ...props }, ref) => {
    ...
    return (
      <PaginationContext.Provider value={context}>
        <View ref={ref} {...props} accessibilityLabel={accessibilityLabel} className={cn('flex-row flex-wrap items-center gap-2', className)}>
          {children}
        </View>
...
type PaginationItemBaseProps = Omit<PressableProps, 'accessibilityRole' | 'children' | 'role'> & {
  children?: React.ReactNode;
  className?: string;
...
type PaginationPageItemProps = PaginationItemBaseProps & {
  /** Page number this item targets, starting at 1. */
  page: number;
  type?: 'page';
};
...
export const PaginationItem = React.forwardRef<React.ComponentRef<typeof Pressable>, PaginationItemProps>(
  ({ accessibilityLabel, accessibilityState, children, className, disabled = false, labelClassName, onPress, page, type = 'page', ...props }, ref) => {
    ...
    const targetPage = type === 'previous' ? pagination.page - 1 : type === 'next' ? pagination.page + 1 : requestedPage;
    ...
    const fallbackContent = type === 'previous' ? previousGlyph : type === 'next' ? nextGlyph : invalidPageItem ? '—' : String(targetPage);
    const inferredLabel = type === 'previous' ? 'Previous page' : type === 'next' ? 'Next page' : invalidPageItem ? 'Page' : `Page ${targetPage}`;
    const webCurrentProps = Platform.OS === 'web' && selected ? ({ 'aria-current': 'page' } as const) : {};

    React.useEffect(() => {
      if (typeof __DEV__ !== 'undefined' && __DEV__ && invalidPageItem) {
        console.warn('BeeUI PaginationItem: `type="page"` requires a finite `page` number. The item is disabled until a valid page is provided.');
      }
    }, [invalidPageItem]);
    ...
    return (
      <Pressable
        ...
        className={cn(
          ...
          selected ? 'border-primary bg-primary' : 'border-border-strong bg-surface web:hover:bg-surface-muted',
      ...
      >
        {(childArray.length > 0 ? childArray : [fallbackContent]).map((child, index) =>
          typeof child === 'string' || typeof child === 'number' ? (
            <Text key={`pagination-item-label-${index}`} className={cn(selected ? 'text-primary-foreground' : 'text-foreground', labelClassName)} variant="label">
              {child}
            </Text>
          ) : child,
        )}
