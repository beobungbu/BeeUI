...
const iconButtonSizeClassName: Record<NonNullable<ButtonProps['size']>, string> = {
  sm: 'h-control-compact w-control-compact px-0 ios:min-h-touch-target android:min-h-touch-target',
...
export const IconButton = React.forwardRef<React.ComponentRef<typeof Button>, IconButtonProps>(
  ({ accessibilityLabel, children, className, count, countClassName, size, ...props }, ref) => {
    ...
    return (
      <View className="relative self-start">
        <Button
          ...
          className={cn(iconButtonSizeClassName[resolvedSize], className)}
          size="icon"
        >
          {children}
        </Button>
