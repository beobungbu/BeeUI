...
- **Defensive `legacy` fallback** — used only when neither context-preserving transport is available: a build where the native `react-native-teleport` host view is unregistered (a stale native build, or JS reloaded onto an app built before the dependency was added) or the New Architecture is unavailable. BeeUI peers React Native `>=0.85`, where the New Architecture is the norm, so this is a capability/stale-build safety net, **not** a recommended deployment. It re-parents content, so consumer context is **not** preserved (resolves to defaults), and a one-time development warning is logged. Recover by running a clean native rebuild so the host view is registered — or, while on the fallback, place required providers at/above `BeeUIProvider` (or pass values in explicitly).

Overlays resolve against the **nearest overlay scope**: the root, or a modal-local scope that `DialogContent` provisions. A scope owns three aligned things — its portal host (so a `Popover`/`DropdownMenu` inside a `Dialog` renders in front of the modal, not behind), its measured geometry origin (so overlays inside a `pageSheet`/`formSheet` position relative to the sheet, not the root window), and its own dismiss stack (so outside press, accessibility escape, web Escape, and hardware back stay scoped — a root overlay behind a modal never steals dismissal from a modal-local child). Platform request-close is routed by platform: **Android** hardware back (which reaches a `Modal` only via `onRequestClose`) is intercepted child-first — back closes the nested overlay first, then the Dialog; **iOS/other** request-close (which can be a native sheet-swipe dismissal) is not intercepted, so React `Dialog` state never desyncs from the native modal. Proven on iOS and Android.

Preserving context on native requires a native rebuild (not an over-the-air JS change); run `expo prebuild --clean` after adding the dependency so the native host view is registered. `react-dom` is optional as BeeUI's own peer (web transport only), but `react-native-teleport` peers on `react-dom`, so a strict package manager may still require a matching `react-dom` even in a native-only app. The `web-dom` transport is proven under Expo Web / the current Metro config (which adds `web` to `resolver.platforms`); portable resolution of the `.web` transport file under arbitrary bundlers and public npm distribution are pre-1.0 distribution-hardening work, not yet guaranteed. Automatic focus restoration and complete VoiceOver/TalkBack keyboard/focus behavior remain runtime/device release gates.
