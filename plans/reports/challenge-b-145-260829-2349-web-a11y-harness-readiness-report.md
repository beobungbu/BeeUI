...
3. **Load-bearing regression** `tests/a11y-readiness.spec.ts` (a11y-audit
   project): MutationObserver installed before opening the Dialog captures any
   roleless `aria-modal` owner; asserts (unconditional) settled contract
   holds post-wait and (lifecycle) every captured transient settled or
   disconnected. Observed `rnw-modal-transient-observed=true` locally — the
   race window is real on every open; the wait closes it.
...
- **All four scenarios now report 0 blocking / 0 allowlisted / 0 non-blocking
...
- Determinism: `--repeat-each=3` on the audit project — 15/15 green, stable
  counts (the 34-vs-33 class of divergence is gone).
- `pnpm typecheck` green. Default visual-web routing unaffected (203 tests,
  no a11y leak; readiness spec runs only under `BEEUI_A11Y_AUDIT=1`).
...
- #280: reclassify/close as harness readiness race per Round 3; no product
  change needed (this branch contains the fix + regression). The
  `1.0:blocker` label belongs to #145's completion, not a component defect.

## Open questions

- The `web-a11y.yml` workflow was cherry-picked as-is; it targets the
  self-hosted runner set used by visual-web. Not exercised here (CI trigger
  is the maintainer's integration step).
